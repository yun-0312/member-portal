<template>
  <div class="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-800">研修会一覧</h1>
        <p class="text-sm text-slate-500 mt-1">開催予定の研修会・説明会をご確認いただけます。</p>
      </div>
            <router-link
              to="/dashboard"
...
const fetchWorkshops = async (page = 1) => {
  ...
  try {
    ...
    // Laravel Paginator の構造に合わせて取得（安全のためフォールバック処理を記述）
...
// 日時表示整形用の関数 (例: "2026/09/24(木) 17:33 〜 19:33")
const formatDateTimeRange = (startStr, endStr) => {
  if (!startStr || !endStr) return ''

  // "2026-09-24 17:33:16" -> Safari等の互換性のためハイフンをスラッシュに置換
