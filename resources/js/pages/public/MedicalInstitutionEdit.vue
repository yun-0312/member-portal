<template>
  <div class="max-w-2xl mx-auto p-4 md:p-6 space-y-6">
    ...
    <div class="border-b border-slate-200 pb-4">
      <h1 class="text-2xl font-bold text-slate-800">医療機関情報の編集</h1>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            required
            class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all bg-white"
            :class="errors.representative_user_id ? 'border-rose-300 bg-rose-50/30' : 'border-slate-300'"
          >
            <option :value="null" disabled>代表者を選択してください</option>
            <option
...
const loadData = async () => {
    ...
    try {
        ...
        api.get(`/medical-institutions/${institutionId}`),
        api.get(`/medical-institutions/${institutionId}/users`)
        ])
        ...
        const rawUsers = membersRes.data?.data || membersRes.data?.users || membersRes.data || []
        memberList.value = rawUsers.filter(user => user.role?.name !== 'medical_staff')
