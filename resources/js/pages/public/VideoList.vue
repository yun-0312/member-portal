        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-4 gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight flex items-center gap-2">
                    <span>🎬</span> 研修・セミナー動画一覧
                </h1>
                <p class="text-xs md:text-sm text-slate-500 mt-1">医療・介護連携に関する研修動画をご視聴いただけます</p>
            </div>
                ...
                >
                    ...
                    <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
                        <div class="space-y-2">
                            ...
                            <h2
                            @click="openModal(video)"
                            class="text-base font-bold text-slate-800 leading-snug cursor-pointer hover:text-blue-600 transition-colors line-clamp-2"
                            >
                            {{ video.title }}
                            </h2>

                            <!-- 説明文（カード側は line-clamp-3 で長さを綺麗に揃える） -->
                            <div
                        ...
                        <div class="space-y-3 pt-3 border-t border-slate-100 text-xs">
                            ...
                            <div class="pt-1 flex justify-end">
                                <!-- 詳細ページリンクボタン -->
                                <router-link
                                    :to="video.show_url || `/videos/${video.id}`"
                                    class="inline-flex items-center justify-center gap-1 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl border border-slate-200 transition-all active:scale-95 shrink-0"
                                    title="詳細ページを開く"
                                >
                                    <span>詳細</span>
                                    <span>→</span>
            ...
            <div class="bg-white rounded-2xl max-w-4xl w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div class="p-4 border-b border-slate-200 flex items-center justify-between">
                    <div class="flex items-center gap-2 pr-4">
                        <span
                        :class="[
                            'px-2 py-0.5 text-[10px] font-bold rounded-md text-white shrink-0',
                    ...
                    <button
                        @click="closeModal"
                        class="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100 transition-colors text-lg font-bold shrink-0"
                        title="閉じる"
                    >
                        ✕
                    </button>
                ...
                <div class="relative aspect-video bg-black shrink-0 flex items-center justify-center">
                    ...
                    <div v-else class="p-8 text-center text-white space-y-4 max-w-md">
                        ...
                        >
                        <span>🎥 外部サイトで動画を視聴する</span>
                        <span>↗</span>
                        </a>
                    </div>
                </div>

                <!-- モーダル内の詳細・説明文・資料表示エリア -->
                <div class="p-5 overflow-y-auto space-y-5 text-xs md:text-sm">
                <!-- 公開期間バッジ -->
                    <div class="flex flex-wrap items-center gap-3 text-xs text-slate-500 bg-slate-50 p-2.5 rounded-xl border border-slate-100 font-mono">
                        <div>📅 公開日: {{ formatDate(selectedVideo.published_at) }}</div>
                        <div v-if="selectedVideo.expired_at">⏳ 掲載終了: {{ formatDate(selectedVideo.expired_at) }}</div>
                    </div>

                    <!-- モーダル内の資料ダウンロード -->
                    <div v-if="selectedVideo.files && selectedVideo.files.length > 0" class="pt-4 border-t border-slate-100 space-y-2">
                        <h4 class="font-bold text-slate-700 text-xs uppercase tracking-wider text-slate-400">添付資料・配布ファイル</h4>
