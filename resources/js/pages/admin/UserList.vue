<template>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div v-else class="space-y-4">
            ...
            <!-- 📱💻 ページネーションエリア -->
            <div v-if="paginationData && paginationData.links" class="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between">
                <!-- 件数情報 -->
                ...
                <!-- 📱 スマホ向け: シンプルな前へ/次へコントロール (sm未満で表示) -->
                <div class="flex sm:hidden items-center justify-between gap-2 pt-2 border-t border-slate-100">
                    <button
                        @click="fetchData(paginationData.prev_page_url)"
                        :disabled="!paginationData.prev_page_url"
                        :class="[
                            'flex-1 py-2 px-3 text-xs font-semibold rounded-xl transition-all text-center',
