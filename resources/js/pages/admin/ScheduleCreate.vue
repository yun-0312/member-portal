<template>
    <div class="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900">スケジュール新規登録</h1>
                <p class="text-xs md:text-sm text-gray-500 mt-1">新しい予定または繰り返しスケジュールを登録します。</p>
            </div>
            <router-link
                :to="indexUrl"
        ...
        <!-- フォーム本体 -->
        <form @submit.prevent="submitForm" class="bg-white rounded-2xl border border-gray-200 shadow-2xs p-6 md:p-8 space-y-6">
                ...
                <div v-else class="space-y-4 bg-blue-50/40 p-4 md:p-5 rounded-xl border border-blue-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div v-if="form.recurrence.frequency === 'monthly'">
                            <label class="block text-xs font-bold text-gray-700 uppercase mb-1">
                                毎月の週位置 (bysetpos) <span class="text-rose-500">*</span>
                            </label>
                            <select
                    ...
                    <!-- 曜日選択 (byweekday) ★ daily と yearly 以外（weekly, monthly）で表示 -->
                    <div v-if="form.recurrence.frequency === 'weekly' || form.recurrence.frequency === 'monthly'">
                    ...
                    <!-- 終了日 (until) -->
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1">繰り返し終了日 (until)</label>
                        <input
        ...
        <div v-if="showSkippedModal" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
                ...
                <p class="text-sm text-gray-600">
                    会議室の重複等の理由により、以下の日時の予定は登録されませんでした。
                </p>
...
onMounted(async () => {
    ...
    try {
        ...
        rooms.value = Array.isArray(roomsRes.data)
            ? roomsRes.data
            : roomsRes.data.data || []
    } catch (err) {
...
const initDefaultDateTime = () => {
    ...
    let baseDateStr = ''
    if (queryDate && typeof queryDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(queryDate)) {
        baseDateStr = queryDate
    } else {
    ...
    // 単発用
    form.start_at = `${baseDateStr}T20:00`
    form.end_at = `${baseDateStr}T21:00`

    // 繰り返し用デフォルト初期値
    form.recurrence.dtstart = baseDateStr
    form.recurrence.start_time = '20:00'
    form.recurrence.end_time = '21:00'
}
...
const submitForm = async () => {
    ...
    if (isRecurring.value) {
        ...
        payload.recurrence = {
            ...
            // daily と yearly の場合は byweekday を null にする
            byweekday: (freq !== 'daily' && freq !== 'yearly') ? form.recurrence.byweekday : null,
            bysetpos: freq === 'monthly' ? form.recurrence.bysetpos : null,
        }

        // バックエンドが start_at / end_at を直接求める実装にも対応できるよう合成値を自動設定
        payload.start_at = `${form.recurrence.dtstart}T${form.recurrence.start_time}`
        payload.end_at = `${form.recurrence.dtstart}T${form.recurrence.end_time}`
    } else {
