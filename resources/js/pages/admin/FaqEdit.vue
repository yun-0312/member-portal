        ...
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 pb-4">
                <div>
                    ...
                    <p class="text-xs md:text-sm text-slate-500 mt-1">登録されているFAQ情報を更新します</p>
                </div>
                <button
                    type="button"
                    @click="goBack"
            ...
            <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-slate-400 gap-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                ...
                <p class="text-sm font-medium">FAQデータを読み込み中…</p>
            </div>
...
// 日時文字列から「YYYY-MM-DD」形式を抽出する関数
const formatToDate = (dateString) => {
    if (!dateString) return ''
    // "2026-07-29T00:00:00.000000Z" または "2026-07-29 00:00:00" のどちらでも前10文字を取得
...
const fetchData = async () => {
    ...
    try {
        ...
        // カテゴリ一覧とFAQ詳細データを並行して取得
        const [catRes, faqRes] = await Promise.all([
            api.get('/admin/faq-categories'),
            api.get(`/admin/faqs/${id}`)
        ])

        categories.value = catRes.data?.items || catRes.data?.data || catRes.data || []

        const item = faqRes.data?.item || faqRes.data?.data || faqRes.data
        if (item) {
            form.received_at = formatToDate(item.received_at)
        ...
        errorMessage.value = 'FAQデータの読み込みに失敗しました。'
    } finally {
        loading.value = false
    }
}
