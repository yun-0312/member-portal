<template>
    <div class="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div class="border-b border-gray-200 pb-4">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">管理者ダッシュボード</h1>
...
const getIcon = (name) => {
    if (name.includes('ユーザー')) return '👤'
    if (name.includes('医療機関')) return '🏥'
    if (name.includes('ロール')) return '🔑'
    if (name.includes('お知らせ')) return '📢'
    if (name.includes('コンテンツ')) return '📁'
    if (name.includes('問い合わせ') || name.includes('faq')) return '💬'
    if (name.includes('スケジュール')) return '📅'
    if (name.includes('会議室')) return '🏢'
